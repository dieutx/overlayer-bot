...
function createFetchRequest(url: string, proxyUrl?: string): FetchRequest {
    ...
    if (proxyUrl) {
        req.getUrlFunc = FetchRequest.createGetUrlFunc({
            agent: new HttpsProxyAgent(formatProxyString(proxyUrl))
        });
    }
...
export async function fetchDailyTasks(address: string, token: string, proxyUrl?: string): Promise<DailyTask[]> {
    ...
    const req = createFetchRequest(
        `${API_BASE}/api-s/socials/onchain-tasks?address=${address}&startDate=${todayStr}&endDate=${todayStr}`,
        proxyUrl
    );
    ...
    return tasks.filter(task => task.startDate === todayStr);
}
...
export async function requestOgMint(
    address: string,
    signature: string,
    message: string,
    proxyUrl?: string
